                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group input-row">
                                {{ Form::label('pagination_size', 'Kích thước phân trang') }}
                                {{ Form::text('pagination_size', $settings['pagination_size'], ['class' => 'form-control']) }}
                            </div>
                            <div class="form-group input-row">
                                {{ Form::label('priority_size', 'Kích thước ưu tiên') }}
                                {{ Form::text('priority_size', $settings['priority_size'], ['class' => 'form-control']) }}
                            </div>
                            <div class="form-group input-row">
                                {{ Form::label('loading_circle', 'Đang tải') }}
                                {{ Form::select('loading_circle', [1 => 'Hiển thị', 0 =>'Không hiển thị'], $settings['loading_circle'], ['class'=>'form-control']) }}
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group input-row">
                                {{ Form::label('currency', 'Loại tiền tệ') }}
                                {{ Form::select('currency', ['PLN' => 'PLN', 'EUR' => 'EUR', 'USD' => 'USD', 'VND' => 'VND'], $settings['currency'], ['class' => 'form-control']) }}
                            </div>
                            <div class="form-group input-row">
                                {{ Form::label('invoice_tax', 'Thuế') }}
                                {{ Form::text('invoice_tax', $settings['invoice_tax'], ['class' => 'form-control']) }}
                            </div>
                        </div>
                        <div class="col-lg-12 validate_form">
                            {{ Form::submit('Lưu cài đặt', ['class' => 'btn btn-primary']) }}
                        </div>
                        {{ Form::close() }}
                    </div>
                </div>
